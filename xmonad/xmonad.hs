...
myLayout = avoidStruts (myGaps ||| tiled ||| Mirror tiled ||| Full ||| grid)
  where
...
--
main = do
    -- xmproc <- spawnPipe "xmobar -r -x 0 ~/.xmobar/xmobarrc"
    xmproc <- spawnPipe "xmobar -r -x 0 ~/code/dotfiles-distrotube/.config/xmobar/xmobarrc0"
    xmonad $ docks $ def {
      -- simple stuff
        terminal           = myTerminal,
      ...
      -- hooks, layouts
        ...
        logHook = workspaceHistoryHook <+> myLogHook <+> dynamicLogWithPP xmobarPP
                        ...
                        , ppVisible = xmobarColor red ""                      -- Visible but not current workspace
                        , ppHidden = xmobarColor base0 "" . wrap "*" ""       -- Hidden workspaces in xmobar
                        , ppHiddenNoWindows = xmobarColor base01 ""           -- Hidden workspaces (no windows)
                        , ppTitle = xmobarColor base01 "" . shorten 60        -- Title of active window in xmobar
                        , ppSep =  "<fc=" ++ base01 ++ "> <fn=1>|</fn> </fc>" -- Separators in xmobar
